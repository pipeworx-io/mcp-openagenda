...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_agendas \
  -H 'Content-Type: application/json' \
  -d '{"query":"Nantes"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_agendas`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "openagenda": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-openagenda"]
    }
  }
}
```
